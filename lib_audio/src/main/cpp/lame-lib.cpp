...
Java_com_sena_audio_LameEncode_init(JNIEnv *env, jobject jclass1, jint sampleRate,
                                     jint channelCount,
...
//    lame_set_VBR_min_bitrate_kbps(gfp, 48);
    //位宽
...
Java_com_sena_audio_LameEncode_encoder(JNIEnv *env, jobject jclass1, jshortArray pcm_buffer,
...
//关闭MP3编码buffer
extern "C" JNIEXPORT jint JNICALL
Java_com_sena_audio_LameEncode_flush(JNIEnv *env, jobject jclass1, jbyteArray mp3_buffer) {
    //获取MP3数组长度
    const jsize mp3_buff_len = env->GetArrayLength(mp3_buffer);
    //获取buffer指针
    jbyte *mp3_buf = env->GetByteArrayElements(mp3_buffer, JNI_FALSE);
...
extern "C" JNIEXPORT void JNICALL
Java_com_sena_audio_LameEncode_writeTag(JNIEnv *env, jobject jclass1, jstring mp3FilePath) {
    FILE  *mp3File = fopen(Jstring2CStr(env, mp3FilePath), "ab+");
...
Java_com_sena_audio_LameEncode_close(JNIEnv *env, jobject type) {
    lame_close(gfp);
