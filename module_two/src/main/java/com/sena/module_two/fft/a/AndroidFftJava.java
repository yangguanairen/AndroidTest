...
/**
 * FileName: AndroidFftJava
 * Author: JiaoCan
 * Date: 2022/12/9 16:41
 */

public class AndroidFftJava {
    ...
    private static int[] fixed_fft(int n, int[] v) {
        int scale = LOG_FFT_SIZE, i, p, r;

        for (r = 0, i = 1; i < n; ++i) {
            // !(p & r)
            for (p = n; (p & r) != 0; p >>= 1, r ^= p);
            if (i < r) {
                int t = v[i];
                v[i] = v[r];
                v[r] = t;
    ...
    public static int[] fixed_fft_real(int n, int[] v) {
    ...
    private static final int LOG_FFT_SIZE = 10;
    private static final int MAX_FFT_SIZE = 1 << LOG_FFT_SIZE;
    private static long[] twiddle = {
